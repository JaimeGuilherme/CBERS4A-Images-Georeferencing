...
def extrair_pontos(preds_binarias, nomes_patches, pasta_imagens):
    dados = []
    for pred, nome_patch in zip(preds_binarias, nomes_patches):
        ...
        for centro in centros:
            ...
            dados.append({'geometry': Point(lon, lat), 'patch': nome_patch,
                          'x_pixel': int(round(x)), 'y_pixel': int(round(y))})
...
if __name__ == '__main__':
    ...
    for pred, nome_patch in zip(results_bin, nomes_patches):
